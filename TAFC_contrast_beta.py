...
from rc2 import rc_time, uwater
import pyrebase
...
# Set Firebase Config                                                
#**************************************************** 
email = 'ntuchenlab@gmail.com'
password = 'abcdefghij'
config = {
    "apiKey": "AIzaSyAO1aWy22W5eE43psrhy9m-r5QQ_-AJsPY",
    "authDomain": "mouseblue-6a0e9.firebaseapp.com",
    "databaseURL": "https://mouseblue-6a0e9.firebaseio.com",
    "storageBucket": "mouseblue-6a0e9.appspot.com",
}
...
blockd = np.random.permutation([55]) # the grating number
    ...
    while True:
        ...
        while True:
            ...
            if count1 < sensi:
                ...
                # firebase recording
                objData = {'TrialNO': int(re_gone + 1),
                'Correction': 1,
                'TrialDu': (second_time - first_time),
                'ITI': first_time - zero,
                'ResponesPos': int(rightLeft),
                'TargetPos': int(rightLeft),
                'grating': str(blockd[num_gone_grating]),
                'reverseSin': str(cad[num_gone]),
                'contrast': int(num_contrast[num_gone_contrast])
                }
                results = db.child('mouse_contrast').child(fm).push(objData, user['idToken'])
            ...
            if count2 < sensi:
                ...
                'TrialDu': (second_time - first_time),
                'ITI': first_time - zero,
                'ResponesPos': int(not rightLeft)*1,
                'TargetPos': int(rightLeft),
                'grating': str(blockd[num_gone_grating]),
                'reverseSin': str(cad[num_gone]),
                'contrast': int(num_contrast[num_gone_contrast])
        ...
        # refresh token
        if (second_time > (Session_init_time + 50*60)):
            user = auth.sign_in_with_email_and_password(email, password)
